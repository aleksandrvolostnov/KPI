...
from app import KanbanColumn, Project, Role, Task, User
...
def test_register_and_login_flow(client, db):
    ...
    resp = client.post(
        "/register",
        data={"username": "newman", "password": "123", "confirm_password": "123", "role_id": str(user_role.id)},
        follow_redirects=True,
    )
    assert resp.status_code == 200
    assert b"Login" in resp.data or b"login" in resp.data.lower()

    resp2 = client.post("/login", data={"username": "newman", "password": "123"}, follow_redirects=True)
...
def test_create_task_api(client, login, db):
    ...
    resp = client.post(
        f"/task/new?project_id={project.id}",
        data={
            "title": "API Task",
            "description": "desc",
...
def test_move_task_on_kanban(client, login, db):
    ...
    response = client.post(
        "/task_board/move",
        data=json.dumps({"task_id": task.id, "new_value": "Done", "mode": "status", "new_position": 0}),
